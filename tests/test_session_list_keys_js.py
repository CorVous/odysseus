...
def test_enter_in_rename_input_is_ignored():
    # Enter commits the rename (handled by the input's own listener); the
    # list-level handler must not also treat it as "open session".
    assert _classify("Enter", tag="INPUT") == "ignore"
...
def test_backspace_on_row_deletes():
    # Focused session row (not an input): Backspace IS the delete shortcut.
    assert _classify("Backspace") == "delete"


def test_delete_on_row_deletes():
